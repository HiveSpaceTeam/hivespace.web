...
const importRouter = async () => {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/scheduled-jobs', name: 'ScheduledJobs', component: { template: '<div />' } },
    ],
  })
...
describe('ScheduledJobsPage', () => {
  ...
  it('should render scheduled jobs page when user has admin role', async () => {
    ...
    expect(router.currentRoute.value.name).toBe('ScheduledJobs')
  })

  it('should redirect non-admin user away from scheduled jobs', async () => {
