...
const importRouter = async () => {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/disputes', name: 'Disputes', component: { template: '<div />' } },
    ],
  })
...
describe('DisputesPage', () => {
  ...
  it('should render disputes page when user has admin role', async () => {
    ...
    expect(router.currentRoute.value.name).toBe('Disputes')
  })

  it('should redirect non-admin user away from disputes', async () => {
