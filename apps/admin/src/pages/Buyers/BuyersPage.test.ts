...
const importRouter = async () => {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/buyers', name: 'Buyers', component: { template: '<div />' } },
    ],
  })
...
describe('BuyersPage', () => {
  ...
  it('should render buyers page when user has admin role', async () => {
    ...
    expect(router.currentRoute.value.name).toBe('Buyers')
  })

  it('should redirect non-admin user away from buyers page', async () => {
