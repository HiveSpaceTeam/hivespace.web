...
const importRouter = async () => {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/kyc-queue', name: 'KycQueue', component: { template: '<div />' } },
    ],
  })
...
describe('KycQueuePage', () => {
  ...
  it('should render KYC queue page when user has admin role', async () => {
    mockGetCurrentUser.mockResolvedValue(createFakeAuthUser({ profile: { role: ['Admin'] } }))
    const router = await importRouter()

    await router.push('/kyc-queue')
    await router.isReady()

    expect(router.currentRoute.value.name).toBe('KycQueue')
  })

  it('should redirect non-admin user away from KYC queue', async () => {
