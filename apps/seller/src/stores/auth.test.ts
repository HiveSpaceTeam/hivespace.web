...
describe('seller auth', () => {
  ...
  it('should grant dashboard access when user has seller role', async () => {
  ...
  it('should redirect when user does not have seller role', async () => {
    // Buyer role with verified email Ã¢â€ â€™ /register-seller
  ...
  it('should load store context on login', () => {
    const fakeUser = createFakeAuthUser({
      profile: { role: ['Seller', 'StoreOwner'], storeId: 'store-001' },
    })

    expect(fakeUser.isSeller()).toBe(true)
    expect((fakeUser.profile as Record<string, unknown>).storeId).toBe('store-001')
  })
})
