...
const mountPage = async ({
  initialPath = '/auth/otp/code?returnUrl=%2Forders%2Fall',
  seed,
}: {
  initialPath?: string
  ...
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/product/list', component: { template: '<div>products</div>' }, meta: { allowAnonymous: true } },
      { path: '/orders/all', component: { template: '<div>orders</div>' }, meta: { allowAnonymous: true } },
    ],
  })
...
describe('OtpCodeEntryPage (seller)', () => {
  beforeEach(() => {
    jest.useFakeTimers()
  ...
  it('submits challengeToken and code then navigates to backend redirectUrl', async () => {
    ...
    const verifySpy = jest
      ...
      .mockResolvedValue({ redirectUrl: '/orders/all', expiresAt: '2026-12-01T00:10:00Z' })
    ...
    await waitFor(() => {
      expect(pushSpy).toHaveBeenCalledWith('/orders/all')
    })
  })

  it('falls back to seller product list when verify response omits redirectUrl', async () => {
    ...
    await waitFor(() => {
      expect(pushSpy).toHaveBeenCalledWith('/product/list')
    })
  })
