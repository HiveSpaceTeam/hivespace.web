<template>
  <AuthLayout :title="t('auth.signIn.title')" :subtitle="t('auth.signIn.subtitle')" :image-src="authImage"
    ...
    <form class="space-y-5" @submit.prevent="handleSubmit">
      <Input id="email" v-model="form.email" type="email" :label="t('auth.signIn.email')"
        :placeholder="t('auth.signIn.emailPlaceholder')" :error="formErrors.email" required />

      <Input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'"
        :label="t('auth.signIn.password')" :placeholder="t('auth.signIn.passwordPlaceholder')"
    ...
    <GoogleAuthButton class-name="w-full" :label="t('auth.google.continue')" :loading="isLoading"
      :on-click="handleGoogleAuth" />

    <p class="mt-5 text-sm text-gray-600 dark:text-gray-400">
      {{ t('auth.signIn.noAccount') }}
      <RouterLink class="font-medium text-brand-500 hover:text-brand-600" to="/signup">
