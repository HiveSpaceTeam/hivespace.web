            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <div class="flex-1">
                  <DateTimePicker v-model="form.startDate"
                    :placeholder="$t('coupon.detail.basicInfo.startDatePlaceholder')" format="iso"
                ...
                <div class="flex-1">
                  <DateTimePicker v-model="form.endDate" :placeholder="$t('coupon.detail.basicInfo.endDatePlaceholder')"
                    format="iso" :disabled="isViewOnly" @update:modelValue="validateEndDateTime" />
              ...
              <div class="mt-4" v-if="form.type !== CouponType.PRIVATE">
                ...
                <div v-if="form.allowEarlySave" class="mt-2 pl-7 max-w-sm">
                  <DateTimePicker v-model="form.earlySaveDate"
                    :placeholder="$t('coupon.detail.basicInfo.earlySaveDatePlaceholder')" format="iso"
          ...
          <div class="flex items-start">
            ...
            <div class="w-3/4 mt-2">
              ...
              <div v-if="isSpecificProducts" class="space-y-4"
                ...
                <div v-if="selectedProductIds.length > 0"
                  ...
                  <table class="w-full text-left bg-white dark:bg-gray-900" style="min-width: 500px">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                      <tr v-for="product in paginatedSelectedProducts" :key="product.id"
                ...
                <div
                  v-if="shouldShowSelectedProductsPagination"
                  class="border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3"
                >
                  <Pagination
                    :currentPage="selectedProductsPage"
                    :totalPages="selectedProductsTotalPages"
                    :pageSize="SELECTED_PRODUCTS_PAGE_SIZE"
                    :totalItems="selectedProductsDetailed.length"
                    :pageSizeOptions="[10]"
                    @update:currentPage="handleSelectedProductsPageChange"
                  />
                </div>
              </div>
            </div>
...
import SelectProductsModal from '@/components/marketing/SelectProductsModal.vue'
...
const SELECTED_PRODUCTS_PAGE_SIZE = 10
const selectedProductsById = ref<Record<number, CouponProductRow>>({})
...
const mapProductRow = (product: Product): CouponProductRow | null => {
...
const mergeSelectedProducts = (products: Product[]) => {
  const nextRows = { ...selectedProductsById.value }

  products.forEach((product) => {
    const mappedProduct = mapProductRow(product)

    if (mappedProduct) {
      nextRows[mappedProduct.id] = mappedProduct
    }
  })

  selectedProductsById.value = nextRows
}

const loadSelectableProducts = async () => {
...
const hydrateMissingSelectedProducts = async (ids: number[]) => {
  const missingIds = ids.filter((id) => !selectedProductsById.value[id])
  ...
  const fetchedProducts = await Promise.all(
    missingIds.map((id) => productStore.fetchProductById(String(id)).catch(() => null)),
  )

  mergeSelectedProducts(
    fetchedProducts.filter((product): product is Product => product !== null),
  )
}
...
const selectedProductsDetailed = computed(() =>
  selectedProductIds.value
    ...
    .filter((product): product is CouponProductRow => product !== undefined)
)

const selectedProductsTotalPages = computed(() =>
  Math.max(1, Math.ceil(selectedProductsDetailed.value.length / SELECTED_PRODUCTS_PAGE_SIZE)),
)

const shouldShowSelectedProductsPagination = computed(() =>
  selectedProductsDetailed.value.length > SELECTED_PRODUCTS_PAGE_SIZE,
)

const paginatedSelectedProducts = computed(() => {
  const start = (selectedProductsPage.value - 1) * SELECTED_PRODUCTS_PAGE_SIZE
  const end = start + SELECTED_PRODUCTS_PAGE_SIZE
...
const handleSelectedProductsPageChange = (page: number) => {
  selectedProductsPage.value = page
}
...
watch(storeProducts, (products) => {
  mergeSelectedProducts(products)
}, { immediate: true })

watch(selectedProductIds, async (ids) => {
  ...
  const hasMissingProducts = ids.some((id) => !selectedProductsById.value[id])

  if (hasMissingProducts) {
    await loadSelectableProducts()
    await hydrateMissingSelectedProducts(ids)
  }
}, { deep: true, immediate: true })

watch(selectedProductsDetailed, (products) => {
  if (products.length === 0) {
    selectedProductsPage.value = 1
    return
  }

  if (selectedProductsPage.value > selectedProductsTotalPages.value) {
    selectedProductsPage.value = selectedProductsTotalPages.value
  }
})
