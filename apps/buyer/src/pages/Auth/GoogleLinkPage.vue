<template>
  <AuthLayout :title="t('auth.googleLink.title')" :subtitle="t('auth.googleLink.subtitle')" :image-src="authImage"
    :image-alt="t('auth.image.alt')" :image-heading="t('auth.image.heading')" :image-body="t('auth.image.body')"
    image-presentation="plain-wide">
    <div v-if="formErrors.common.length"
      class="mb-5 rounded-lg bg-red-50 p-4 text-sm text-red-700 dark:bg-red-500/10 dark:text-red-300" role="alert">
      <p v-for="message in formErrors.common" :key="message">{{ message }}</p>
    </div>

    <div class="mb-5 rounded-lg bg-blue-50 p-4 text-sm text-blue-800 dark:bg-blue-500/10 dark:text-blue-200">
    ...
    <form class="space-y-5" @submit.prevent="handleConfirm">
      ...
      <p v-if="formErrors.consentAccepted" class="form-error-text mt-1 text-sm text-red-600 dark:text-red-400">
        {{ formErrors.consentAccepted }}
      </p>

      <Input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'"
        :label="t('auth.googleLink.password')" :placeholder="t('auth.googleLink.passwordPlaceholder')"
        :error="formErrors.password" inputClass="pr-10" required>
        <template #append>
          <button type="button" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400" :aria-label="showPassword ? t('auth.actions.hidePassword') : t('auth.actions.showPassword')
      ...
      <Button type="submit" class="w-full" :disabled="!form.consentAccepted" :loading="isLoading">
        {{ t('auth.googleLink.confirm') }}
      </Button>
      <Button type="button" variant="outline" class="w-full" :disabled="isLoading" :on-click="handleCancel">
    ...
    <div class="mt-5 space-y-2 text-sm">
      ...
      <button type="button" class="font-medium text-brand-500 hover:text-brand-600" :disabled="isLoading"
